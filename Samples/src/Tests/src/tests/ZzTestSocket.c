 ...
 */
rt_un32 RT_CALL ZzServerSocketThreadCallback(void *parameter)
{
...
rt_s RT_CALL ZzTestSockets()
{
	...
	if (!rt_thread_create(&thread, &ZzServerSocketThreadCallback, &event)) goto error;
	...
	if (!rt_socket_connect_with_socket_address(&zzSocket, (struct rt_socket_address*)&zzSocketAddress)) goto error;
