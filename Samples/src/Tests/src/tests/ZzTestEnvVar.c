...
RT_B RT_CALL ZzTestEnvVar()
{
  ...
  if (RtEnvVar_Get(_R("Non-existing name"), lpBuffer, RT_CHAR_HALF_BIG_STRING_SIZE, &unOutputSize)) goto handle_error;
  ...
  if (!RtEnvVar_Get(lpVariableName, lpBuffer, RT_CHAR_HALF_BIG_STRING_SIZE, &unOutputSize)) goto handle_error;

  if (!RtChar_GetStringSize(lpBuffer)) goto handle_error;

  if (!RtEnvVar_Set(_R("RT_FOO"), _R("BAR"))) goto handle_error;

  if (!RtEnvVar_Get(_R("RT_FOO"), lpBuffer, RT_CHAR_HALF_BIG_STRING_SIZE, &unOutputSize)) goto handle_error;
