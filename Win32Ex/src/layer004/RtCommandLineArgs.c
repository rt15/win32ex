    ...
    if (lpArg[0] == '-')
    {
        ...
        else
        {
          ...
          *lpArgType = RT_COMMAND_LINE_ARG_TYPE_LONG;
          ...
          if (unI != unArgSize)
          {
            ...
            if (!RtChar_CopyStringWithSize(&lpArg[2], unI - 2, lpLongOptionBuffer, unLongOptionsBufferSize, lpLongOptionsWritten)) goto handle_error;
          ...
          else
          {
            ...
            if (!RtChar_CopyString(&lpArg[2], lpLongOptionBuffer, unLongOptionsBufferSize, lpLongOptionsWritten)) goto handle_error;
      ...
      else
      {
        ...
        *lpArgType = RT_COMMAND_LINE_ARG_TYPE_SHORT;
        if (!RtChar_CopyString(&lpArg[1], lpShortOptionsBuffer, unShortOptionsBufferSize, lpShortOptionsWritten)) goto handle_error;
        *lpValue = RT_NULL;
      }
    }
    else
...
RT_B RT_API RtCommandLineArgs_Parse(RT_N32* lpArgC, RT_CHAR* lpArgV[], RT_COMMAND_LINE_ARGS_CALLBACK lpCommandLineArgsCallback, void* lpContext,
                                    ...
                                    RT_N32* lpNonOptionsIndex)
{
  ...
  *lpNonOptionsIndex = RT_TYPE_MAX_N32;
  ...
  for (nI = 1; nI < *lpArgC; nI++)
  {
    ...
    {
      if (RtChar_SearchStringInStrings(lpLongOptionsWithoutArg, lpLongOptionBuffer) != RT_TYPE_MAX_UN)
      {
        bValid = RT_TRUE;
        unValueCardinality = RT_COMMAND_LINE_ARG_VALUE_NONE;
      }
      else if (RtChar_SearchStringInStrings(lpLongOptionsWithOptionalArg, lpLongOptionBuffer) != RT_TYPE_MAX_UN)
      {
        bValid = RT_TRUE;
        unValueCardinality = RT_COMMAND_LINE_ARG_VALUE_OPTIONAL;
      }
      else if (RtChar_SearchStringInStrings(lpLongOptionsWithArg, lpLongOptionBuffer) != RT_TYPE_MAX_UN)
      {
        bValid = RT_TRUE;
        unValueCardinality = RT_COMMAND_LINE_ARG_VALUE_REQUIRED;
    ...
    else if (unArgType == RT_COMMAND_LINE_ARG_TYPE_NON_OPTION)
    {
      ...
      if (nNonOptions == RT_TYPE_MAX_N32)
      {
        ...
        for (nK = nI + 1; nK < *lpArgC; nK++)
        {
          ...
          {
            if (RtChar_SearchStringInStrings(lpLongOptionsWithArg, lpLongOptionBuffer) != RT_TYPE_MAX_UN)
            {
              if (!lpValue)
              {
