...
static RT_B RT_API RtShortSort(char* lo, char* hi, RT_UN unItemSize, RT_COMPARISON_CALLBACK lpComparisonCallback, void* lpContext)
{
    char *p, *max;
    ...
    while (hi > lo) {
        ...
        RtSwapMemory(max, hi, unItemSize);
...
recurse:
    ...
    if (size <= 8) {
        if (!RtShortSort(lo, hi, unItemSize, lpComparisonCallback, lpContext))
        {
          return RT_FALSE;
        }
    }
        ...
        if (nComparisonResult > 0) {
            RtSwapMemory(lo, mid, unItemSize);
        }
        ...
        if (nComparisonResult > 0) {
            RtSwapMemory(mid, hi, unItemSize);
        }
        ...
        for (;;) {
            ...
            RtSwapMemory(loguy, higuy, unItemSize);
