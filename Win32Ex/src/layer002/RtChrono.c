 ...
 */
void RT_CALL RtChrono_SubstractTimespecs(struct timespec *lpX, struct timespec *lpY, struct timespec *lpResult)
{
  struct timespec rtY;
  int nSec;
...
RT_B RT_API RtChrono_GetDuration(RT_CHRONO* lpChrono, RT_UN* lpMicroSeconds)
{
  ...
  *lpMicroSeconds = (RT_UN)((rtCounterValue.QuadPart - lpStartCounter->QuadPart) * 1000000 / rtFrequency.QuadPart);
  ...
  RtChrono_SubstractTimespecs(&rtCounterValue, lpStartCounter, &rtDifference);
